...
START_TEST(test_deterministic_key_pair_iterator)
{
    ...
    strcpy(seed, "seed");
    ck_assert_int_eq(true, deterministic_key_pair_iterator((const uint8_t*)seed, strlen(seed), next_seed, seckey, pubkey));
    ...
    memset(pubkey, 0, sizeof(pubkey));
    memset(seckey, 0, sizeof(seckey));
    ck_assert_int_eq(true, deterministic_key_pair_iterator((const uint8_t*)seed, 32, next_seed, seckey, pubkey));
    ...
    memset(pubkey, 0, sizeof(pubkey));
    memset(seckey, 0, sizeof(seckey));
    ck_assert_int_eq(true, deterministic_key_pair_iterator((const uint8_t*)seed, 64, next_seed, seckey, pubkey));
...
START_TEST(test_deterministic_key_pair_iterator_seed_lenght)
{
    char seed[256] = {0};
    uint8_t seckey[32] = {0};
    uint8_t pubkey[33] = {0};
...
Suite* test_suite(void)
{
    ...
    tcase_add_test(tc, test_deterministic_key_pair_iterator_seed_lenght);
